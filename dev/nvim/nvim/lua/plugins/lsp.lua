return {
  ...
  config = function()
    ...
    local on_attach = function(client, bufnr)
      ...
      local keybinding = "<Leader>cf"
      -- Check if the file type is Lua
      local filetype = vim.api.nvim_buf_get_option(bufnr, "filetype")
      if filetype == "lua" then
        -- Set up the custom formatting command for Lua files
        if client.server_capabilities.documentFormattingProvider then
          vim.api.nvim_buf_set_keymap(bufnr, "n", keybinding,
                                      "<cmd>lua require(\"core.utils\").custom_lua_format()<CR>",
                                      { noremap = true, silent = true })
        end
      else
        -- Set up the default formatting command for other file types
        if client.server_capabilities.documentFormattingProvider then
          vim.api.nvim_buf_set_keymap(bufnr, "n", keybinding,
